...
import type { LoginResponse, SignInParams } from '@/core/modals/userT';
import { cn } from '@/lib/utils';
import { faEyeSlash, faEye } from '@fortawesome/free-solid-svg-icons';
import  { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import  { Button } from '../ui/button';
import  { Card, CardContent } from '../ui/card';
import  { FieldGroup, Field, FieldLabel, FieldDescription, FieldSeparator } from '../ui/field';
...
const SignInForm: React.FC = () => {
        ...
        const submission: SubmitHandler<SignInParams> = async ( data: SignInParams) => {
                                ...
                                // setTimeout(()=>{
                                        if(loginResponse && loginResponse.user){
                                                const userData = {
                                                        ...
                                                        name: loginResponse.user.first_name, 
                                        ...
                                        if(!response.success){
                                                ...
                                                toast.error(`Login failed. ${error.errorMessage || "Please try again."}`);
                                                console.log("LoginNotSuccess", response);
                                        }
                                        setLoading(false);
                                // }, 2000);
         ...
         useEffect(()=>{
            ...
            },[auth, navigate, callApi]); 

  return (
    <div className="mx-auto max-w-300 py-6">
        {
                loading && (<FullScreenLoader message='Wait, Loggin you In...'/>)
        }
